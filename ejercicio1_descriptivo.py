...
# En concreto con la columna daily_social_media_time vamos a usar su mediana ya que tiene un skewness muy alto (1.25) y una kurtosis de 6.04:
mediana_dsmt = df["daily_social_media_time"].median()
df["daily_social_media_time"] = df["daily_social_media_time"].fillna(mediana_dsmt)
# Para los demás rellenamos con la media, usamos el parametro numeric_only=True para que solo
# haga las medias con las columnas numéricas:
media = df.mean(numeric_only=True)
# Finalmente rellenamos los nulos:
df = df.fillna(media)
...
for row in upper.index:
    for col in upper.columns:
        val = upper.loc[row, col]  # Mira los valores exactos de la columna y la fila
        if val > 0.9:
            pares = True
