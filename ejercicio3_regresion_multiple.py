...
def regresion_lineal_multiple(X_train, y_train, X_test):
    ...
    # Creación de la columna de '1' con el mismo numero de filas que X_train
    unos = np.ones((X_train.shape[0], 1))
    # Pegamos la columna de '1' a la izquierda de X_train
    X_train_b = np.hstack([unos, X_train])  # ← Reemplaza None con tu implementación

    # TODO: Paso 2 — Calcular los coeficientes β con la fórmula OLS
    # β = (XᵀX)⁻¹ Xᵀy
    # Multiplicamos la matriz de X_train_b transpuesta con su matriz original
    X_matriz = X_train_b.T @ X_train_b
    # Lo invertimos
    X_matriz_inv = np.linalg.inv(X_matriz)
    # Multiplicamos la matrix X_train_b transpuesta con la matriz objetivo
    XY_matriz = X_train_b.T @ y_train
    # Ahora multiplicamos X_matriz_inv con XY_matriz
    coefs = X_matriz_inv @ XY_matriz  # ← Reemplaza None con tu implementación

    # TODO: Paso 3 — Añadir columna de unos a X_test de la misma forma
    # Añadimos la misma columna que hemos generado antes a X_test
    unos_test = np.ones((X_test.shape[0], 1))
    X_test_b = np.hstack([unos_test, X_test])  # ← Reemplaza None con tu implementación

    # TODO: Paso 4 — Calcular predicciones ŷ = X_test_b · β
    # Implementamos los coeficientes que hemos generado antes
...
def graficar_real_vs_predicho(
    y_real, y_pred, ruta_salida="output/ej3_predicciones.png"
):
    """
    ...
    valor_min = min(y_real.min(), y_pred.min())
    valor_max = max(y_real.max(), y_pred.max())
...
if __name__ == "__main__":
    ...
    with open("output/ej3_coeficientes.txt", "w", encoding="utf-8") as f:
        ...
        f.write("=" * 50 + "\n")
        nombres = ["Intercepto (β₀)"] + [
            f"β{i+1} (feature {i+1})" for i in range(n_features)
        ]
