...
def generar_serie_temporal(semilla=42):
    ...
    estacionalidad = 15 * np.sin(2 * np.pi * t / 365.25) + 6 * np.cos(
        4 * np.pi * t / 365.25
    )
...
def visualizar_serie(serie):
    ...
    serie.plot(ax=ax, color="teal")
    plt.title("Evolución de la serie temporal 2018-2023")
...
def analizar_residuo(residuo):
    ...
    plt.hist(residuo_limpio, bins=30, density=True, alpha=0.6, color="g")
    # Curva normal teórica con la media y estandar que calculamos antes
    xmin, xmax = plt.xlim()
